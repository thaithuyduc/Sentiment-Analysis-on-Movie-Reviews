...
class GRUCell(nn.Module):
    def __init__(self, emb_dim:int, hidden_size:int, num_layers:int, dropout:float,
                bidirectional:bool, pooling_fn: Literal['mean', 'sum', 'max', 'attention']|None = None):
        super().__init__()
    ...
    def forward(self, tokens_emb, mask):
        ...
        # we must tell RNNs to avoid these values, achieve this using pack_padded_sequence
...
class GRUModel(nn.Module):
    def __init__(self, num_classes:int, vocab:dict[str, int], emb_dim:int, hidden_size:int, num_layers:int,
        ...
        self.gru = GRUCell(emb_dim=emb_dim, hidden_size=hidden_size, num_layers=num_layers, 
                            bidirectional=bidirectional, dropout=0, pooling_fn=pooling_fn)
    ...
    def forward(self, tokens_idx):
        ...
        last_hidden_state = self.gru(tokens_emb, mask) # (B, bidirections * hidden_size) or (B, hidden_size)
