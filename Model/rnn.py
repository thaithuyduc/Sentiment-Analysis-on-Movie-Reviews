...
class RNNCell(nn.Module):
    def __init__(self, emb_dim:int, hidden_size:int, num_layers:int, 
                 nonlinearity: Literal['relu', 'tanh'], bidirectional:bool, 
                 dropout_prob:float, pooling_fn: Literal['mean', 'sum', 'max', 'attention']|None):
        super().__init__()
        self.rnn = nn.RNN(input_size=emb_dim, hidden_size=hidden_size, num_layers=num_layers,
                            nonlinearity=nonlinearity, bias=True, batch_first=True, dropout=dropout_prob, 
    ...
    def forward(self, tokens_emb, mask):
        # Since there are some sequence having Padding index, 
        # we must tell RNN to avoid these indices, achieve this using pack_padded_sequence
        ...
        # output_packed: PackedSequence object ; last_hidden_state: (bidirections * num_layers, B, hidden_size)
...
class RNNModel(nn.Module):
    def __init__(self, num_classes:int, vocab:dict[str, int], emb_dim:int, hidden_size:int, num_layers:int,
                bidirectional:bool, pooling_fn: Literal['mean', 'sum', 'max', 'attention']|None):
        super().__init__()
        self.vocab = vocab
        ...
        self.rnn = RNNCell(emb_dim=emb_dim, hidden_size=hidden_size, num_layers=num_layers, 
                           nonlinearity='relu', bidirectional=bidirectional, dropout_prob=0, 
    ...
    def forward(self, tokens_idx):
        ...
        last_hidden_state = self.rnn(tokens_emb, mask) # (B, bidirections * hidden_size) or (B, hidden_size)
